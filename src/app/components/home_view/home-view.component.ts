...
export class HomeViewComponent {
  ...
  ngOnInit() {
    ...
    const that = this;
    this.dropzone = new Dropzone(this.dzRef.nativeElement, {
      url: environment.fileupload_url + "?tag=" + this.sessionKey,
      maxFilesize: 9.98,
      clickable: '.dropzone-elements',
      init: function () {
        this.on("success", function(file, res) {
          that.fileUploads.push(res);
          that.addedFilesNow.push(res);
          that.store.dispatch(new GetStatus(that.sessionKey));
        });
      }
    });
  }
